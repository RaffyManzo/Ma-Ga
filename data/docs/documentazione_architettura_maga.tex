...
conto delle correzioni recenti, in particolare della sorgente filtrata e delle
...
\begin{tikzpicture}[node distance=1.15cm and 1.0cm]
    ...
    \node[box, right=of loader] (source) {\cls{SystemStateSource}};
    \node[box, below=of source] (filtered) {\cls{FilteringSystemStateSource}};
    \node[policy, left=of filtered] (prefilter) {\cls{CandidatePrefilter}};

    \node[widebox, right=1.45cm of source] (manager) {\cls{TemporalWindowManager}};
    ...
    \draw[arrow] (loader) -- (source);
    \draw[arrow] (source) -- (manager);
    \draw[dashedarrow] (prefilter) -- (filtered);
    \draw[dashedarrow] (source) -- (filtered);
...
\addlinespace
Stato $S_k$ &
\cls{SystemSnapshot}. Contiene veicoli, task, candidati, gateway radio, access link e pool di banda disponibili nello stesso istante. \\
\addlinespace
Task $i$ &
\cls{TaskInstance}. Porta id, veicolo sorgente, input, output, cicli CPU e deadline. \\
\addlinespace
Nodo candidato $j$ &
\cls{NodeCandidate}. Descrive nodo locale, veicolo remoto, edge o cloud. Include CPU, banda, latenza, compatibilita' con il sorgente e binding opzionale al pool di banda. \\
\addlinespace
Decisione $x_{ij}$ &
...
\cls{SystemSnapshot} e' il contenitore dello stato. Ha sei liste principali:

\begin{itemize}[leftmargin=*,nosep]
    \item veicoli, tramite \cls{VehicleSnapshot};
    \item task, tramite \cls{TaskInstance};
    \item candidati, tramite \cls{NodeCandidate};
    \item gateway radio, tramite \cls{AccessGatewaySnapshot};
    \item access link veicolo-gateway, tramite \cls{AccessLinkSnapshot};
    \item pool di banda condivisa, tramite \cls{BandwidthPoolSnapshot}.
\end{itemize}

Il GA riceve sempre uno \cls{SystemSnapshot}. Questo evita dipendenze dirette da
JSON, file o sorgenti temporali.
...
\clsb{AccessGatewaySnapshot} &
Descrive un gateway radio osservato, con posizione, raggio e pool di banda opzionale. \\
\clsb{AccessLinkSnapshot} &
Associa veicolo e gateway; indica il link attivo e la disponibilita' dichiarata. \\
\clsb{BandwidthPoolSnapshot} &
Descrive il budget di banda condiviso: globale, per gateway o diretto V2V. \\
\clsb{CoverageEstimator} &
Stima per quanto tempo il sorgente resta coperto dal candidato. Per CLOUD usa l'access gateway attivo. La fitness usa questa stima per la penalita' di mobilita'. \\
\bottomrule
\end{tabularx}
...
\section{Caricamento e sorgenti}

\cls{SnapshotLoader} legge gli snapshot dai file. Prima mappa il JSON nel
domain model completo e poi \cls{SnapshotValidator} controlla che lo scenario
sia coerente: ID univoci, riferimenti validi, fallback locale per task, almeno
un pool di banda e un solo access link attivo per veicolo.

Il gestore temporale non conosce i file. Dipende solo da \cls{SystemStateSource}.
...
\subsection{Sorgente filtrata}

\cls{FilteringSystemStateSource} e' un decorator. Riceve una sorgente reale e
un \cls{CandidatePrefilter}. Quando il manager chiede un'osservazione, il decorator:

\begin{enumerate}[leftmargin=*,nosep]
    \item chiama la sorgente delegata con \meth{nextObservation};
    \item applica il prefilter allo snapshot contenuto nell'osservazione;
    \item salva il risultato diagnostico;
    \item restituisce un'osservazione con snapshot filtrato e metadati temporali invariati.
\end{enumerate}

La correzione recente e' questa: il package provider storico e'
stato rimosso dal codice. Il contratto unico e' \cls{SystemStateSource}, che porta insieme
snapshot, tempo richiesto, tempo sorgente e modalita' della sorgente.
...
Dopo il repair del singolo gene entrano i repair aggregati. La sequenza corrente
controlla:

\begin{itemize}[leftmargin=*,nosep]
    \item CPU aggregata per \code{executionNodeId}, tramite \cls{CpuAggregateRepairOperator};
    \item banda aggregata per singolo link \code{candidateId}, tramite \cls{BandwidthAggregateRepairOperator};
    \item banda aggregata per pool condiviso \code{poolId}, tramite \cls{BandwidthPoolAggregateRepairOperator}.
\end{itemize}

La banda non e' piu' una parte aperta del repair. Resta da validare
sperimentalmente quanto il ridimensionamento proporzionale link+pool sia
competitivo rispetto a policy piu' sofisticate.
...
Nel caso parziale, la quota remota $p_i$ scala tutte le componenti del ramo
remoto:

\[
T_i^{upload} = \frac{p_i \cdot input_i}{b_i}
\qquad
T_i^{exec} = \frac{p_i \cdot cycles_i}{f_i}
\qquad
T_i^{download} = \frac{p_i \cdot output_i}{b_i}
\]

Il ramo locale usa la parte complementare:

\[
T_i^{local} = \frac{(1-p_i)\cdot cycles_i}{f_i^{local}}
\]
...
La mobilita' usa rischio di copertura, instabilita' del link e rischio di
handover. Per CLOUD copertura e \code{phi\_link} derivano dall'access gateway
attivo; \code{CLOUD\_STABLE\_PLACEHOLDER} resta solo come compatibilita'
storica e non e' usato in modalita' \code{STRICT\_GATEWAY}.

\subsection{Risorse}

La CPU remota viene aggregata per \code{executionNodeId}. La banda viene
controllata su due livelli: per \code{candidateId}, cioe' il link source-aware,
e per \code{poolId}, cioe' il budget condiviso risolto da
\cls{BandwidthPoolResolver}. Un pool \code{GLOBAL} riproduce il $B_{max}$ unico;
pool \code{GATEWAY} e \code{DIRECT\_V2V} specializzano domini radio separati.
...
Le componenti di dinamicita' sono state riallineate alla formalizzazione:
\code{Dv(k)} misura solo il churn dei veicoli, mentre \code{Dl(k)} misura la
variazione media della qualita' degli access link attivi,
$q_v(k)=1-\phi_{link}(v,gatewayAttivo)$, sui veicoli presenti in due finestre
consecutive.
...
\midrule
Finestra temporale &
\meth{run} chiama \meth{executeNextStepOrNull}. Dentro lo step compaiono \meth{resolveTrigger}, \meth{nextObservation}, \meth{evaluate}, \meth{adaptPopulation} e \meth{optimizeDetailed}. Questo percorso decide quando ottimizzare e con quale popolazione partire. \\
\addlinespace
Sorgente filtrata &
\meth{nextObservation} chiama la sorgente delegata, poi \meth{filterObservation}. Il filtro usa \meth{CandidatePrefilter.filter} e restituisce \meth{getFilteredSnapshot}. Questo percorso inserisce il prefilter senza cambiare il manager. \\
\addlinespace
Inizializzazione &
...
\midrule
\clsb{SystemStateSource} &
Fornisce l'osservazione richiesta dal manager, includendo snapshot e metadati temporali. \\
\clsb{FilteringSystemStateSource} &
Applica il prefilter prima di restituire l'osservazione al manager. \\
...
\addlinespace
Instabilita' link &
La variabile e' calcolata dalle metriche di mobilita'. Per CLOUD deriva dall'access gateway attivo; per V2V resta una stima scalare senza vettori di traiettoria. \\
\addlinespace
...
\addlinespace
Prefilter &
Nel codice e' gia' integrato tramite \cls{FilteringSystemStateSource}. Riduce candidati prima del GA. \\
\addlinespace
Quote e risorse &
\cls{OffloadingRatioPolicy} e \cls{ResourceAllocationPolicy} guidano il campionamento. La fitness resta il giudice finale. \\
\addlinespace
Banda aggregata &
La fitness e il repair la trattano su due livelli: link \code{candidateId} e pool condiviso \code{poolId}. \\
\bottomrule
\end{tabularx}
...
\begin{enumerate}[leftmargin=*]
    \item \cls{SystemSnapshot}, \cls{TaskInstance}, \cls{VehicleSnapshot}, \cls{NodeCandidate}, \cls{AccessGatewaySnapshot}, \cls{AccessLinkSnapshot}, \cls{BandwidthPoolSnapshot}.
    ...
    \item \cls{CandidatePrefilter}, \cls{BandwidthPoolResolver} e \cls{DeadlineViolationAnalyzer}.
\end{enumerate}
...
Le correzioni recenti rendono il sistema piu' coerente. La sorgente filtrata ora
rispetta il contratto unico di \cls{SystemStateSource} e propaga anche gateway,
access link e pool di banda. Le nuove policy di quota e risorse evitano molte
scelte casuali deboli senza togliere esplorazione al GA. Il repair aggregato
copre CPU, banda per link e banda per pool condiviso.
