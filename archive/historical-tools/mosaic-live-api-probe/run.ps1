...
if (-not (Test-Path -LiteralPath (Join-Path $ResolvedMosaicRoot "scenarios\MaGaLiveBridgeProbe") -PathType Container)) {
    throw "Probe scenario is not deployed under $ResolvedMosaicRoot"
}

Push-Location $ResolvedMosaicRoot
try {
    Write-Host "Running: .\mosaic.bat -s MaGaLiveBridgeProbe"
