...
$VersionedScenario = Join-Path $RepoRoot "data\mosaic-scenarios\MaGaLiveBridgeProbe"
...
$JarFile = Join-Path $OutRoot "maga-live-api-probe.jar"
...
if (-not (Test-Path -LiteralPath $ScenarioApplicationDir -PathType Container)) {
    throw "Probe scenario application directory not found: $ScenarioApplicationDir"
}
...
Write-Host "Compiling MaGa live API probe classes..."
...
$ExpectedClassFiles = @(
    "org\eclipse\mosaic\app\maga\liveprobe\MaGaLiveProbeConfig.class",
    "org\eclipse\mosaic\app\maga\liveprobe\MaGaLiveVehicleApiProbeApp.class",
    "org\eclipse\mosaic\app\maga\liveprobe\MaGaLiveCoordinatorApiProbeApp.class"
)
...
$ScenarioJar = Join-Path $ScenarioApplicationDir "maga-live-api-probe.jar"
Copy-Item -LiteralPath $JarFile -Destination $ScenarioJar -Force
Write-Host "Copied live probe JAR to versioned scenario: $ScenarioJar"
