#!/usr/bin/env python3
"""Checkpoint-aware orchestrator for the MA-GA final test campaign G00."""
...
DEFAULT_SPEC = SCRIPT_DIR / "final_campaign_spec.json"
SUBSCENARIO_NAME = "intas_literature_urban"
SCENARIO_NAME = "MaGaLiteratureBasedUrbanStudy"

sys.path.insert(0, str(SCRIPT_DIR))
from validate_final_campaign import validate_scenario  # noqa: E402
...
def archive_campaign_root(repo_root: Path, spec: dict[str, Any]) -> dict[str, Any]:
    ...
    destination = (repo_root / spec["paths"]["archiveRoot"] / f"final-test-campaign-pre-g00-{timestamp}").resolve()
...
def update_config_mapping(repo_root: Path, spec: dict[str, Any]) -> None:
    ...
    for row in rows:
        ...
        if config_id in {"CFG-M-BACKGROUND", "CFG-M-RSU0", "CFG-M-RSU1", "CFG-M-SWITCH", "CFG-M-V2V", "CFG-R-V2VBW"}:
            ...
            row["notes"] = append_note(row["notes"], "G00: direct route-family vehicle counts resolved in final_campaign_spec.json; profile remains non-literature-calibrated and excluded from main factorial claims")
...
def update_docs(repo_root: Path, spec: dict[str, Any], summary: dict[str, Any]) -> None:
    ...
    body = (
        ...
        "Tooling created under `tools/intas-literature-scenario/final_campaign/`. "
        "Concrete scenarios remain under `tmp/materialized-literature-scenarios/final-test-campaign/`; MOSAIC was not executed. "
...
def collect_archived_failure_anomalies(repo_root: Path) -> tuple[list[dict[str, str]], list[Path]]:
    archived_failures = {
        "MAT-CFG-SMOKE-104729": (
            ...
            / "tmp"
            / "archive"
            / "final-test-campaign-pre-g00-20260620-104042"
        ...
        "MAT-CFG-M-RSU1-104729": (
            ...
            / "tmp"
            / "archive"
            / "final-test-campaign-pre-g00-20260620-105205"
...
def first_manifest_plan_hash(repo_root: Path, plan: list[dict[str, str]]) -> str:
    for row in plan:
        ...
        for item in manifest.get("sourceFiles", []):
            if item.get("path") == "data/docs/testing/final-campaign/scenario_instance_plan.csv":
...
def create_audit(repo_root: Path, spec: dict[str, Any], plan: list[dict[str, str]], summary: dict[str, Any]) -> None:
    ...
    section_lines = [
        ...
        "",
        f"- frozen_branch: `MOSAIC/SUMO-integration`",
    ...
    add_evidence("EV-G00-TOOL-SPEC", SCRIPT_DIR / "final_campaign_spec.json", "manifest", "Campaign specification.", test_ids=["T-001"])
    add_evidence("EV-G00-TOOL-ORCHESTRATOR", SCRIPT_DIR / "materialize_final_campaign.py", "summary", "Campaign orchestrator.", test_ids=["T-001"])
    add_evidence("EV-G00-TOOL-VALIDATOR", SCRIPT_DIR / "validate_final_campaign.py", "validator", "Campaign validator.", test_ids=["T-002"])
    add_evidence("EV-G00-TOOL-POWERSHELL", SCRIPT_DIR / "materialize_final_campaign.ps1", "command-output", "PowerShell wrapper.", test_ids=["T-001"])
