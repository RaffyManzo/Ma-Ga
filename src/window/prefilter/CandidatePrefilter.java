...
public final class CandidatePrefilter {
    ...
    public CandidatePrefilter(CandidatePrefilterConfig config, MobilityConfig mobilityConfig) {
        ...
        if (mobilityConfig == null) {
            throw new IllegalArgumentException("mobilityConfig must not be null.");
        }
        this.config = config;
    ...
    public CandidateFilteringResult filter(SystemSnapshot snapshot) {
        ...
        /*
         * IMPORTANTE:
         * il prefilter riduce soltanto candidateNodes. Tutte le strutture
         * osservate necessarie agli stadi successivi devono essere propagate:
         * gateway, access link e pool di banda condivisi.
         */
    ...
    private CandidateDecision evaluate(
        ...
        if (!taskSources.contains(candidate.getSourceVehicleId())) {
            return CandidateDecision.reject(
                    ...
                    0.0,
                    "No active task for source vehicle."
            );
        }
        ...
        if (!Double.isFinite(candidate.getAvailableBandwidth())
            ...
            return CandidateDecision.reject(
                    ...
                    0.0,
                    "Remote bandwidth must be > 0."
            );
        }

        if (candidate.getType() == NodeType.CLOUD) {
            try {
                AccessLinkMetrics access = accessLinkMetricsEstimator
                        .estimateActiveLink(snapshot, candidate.getSourceVehicleId());
                if (!access.isAvailable()
                        || access.getCoverageTimeSeconds() <= 0.0) {
                    return CandidateDecision.reject(
                ...
                return CandidateDecision.keep(
                        access.getCoverageTimeSeconds(),
                        "CLOUD candidate preserved through active gateway "
                                + access.getGatewayId() + "."
                );
        ...
        if (!Double.isFinite(coverage) || coverage <= 0.0) {
            return CandidateDecision.reject(
                    CandidateRejectionReason.INSUFFICIENT_COVERAGE,
                    coverage,
                    "Remote candidate outside physical coverage."
            );
        }
        return CandidateDecision.keep(
                coverage,
    ...
    private double estimateNonCloudCoverage(
        ...
        if (candidate.getType() == NodeType.EDGE) {
            ...
            return (radius - distance)
                    / Math.max(EPSILON, Math.abs(source.getSpeed()));
        }
    ...
    private static final class CandidateDecision {
        ...
        private CandidateDecision(
            ...
            this.keep = keep;
            this.reason = reason;
            this.coverageSeconds = coverageSeconds;
            this.note = note;
        }

        private static CandidateDecision keep(double coverage, String note) {
        ...
        private static CandidateDecision reject(
            ...
            return new CandidateDecision(false, reason, coverage, note);
        }
    }
}
